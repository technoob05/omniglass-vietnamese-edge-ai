...
function showTurn(j){const r=j.response||j;answer.textContent=r.answer_vi||r.error||'—';facts.textContent=JSON.stringify({source:r.source,thermal_wait_ms:r.thermal_wait_ms,vlm_latency_ms:r.latency_ms,frame:r.vlm_input,yolo_depth_scene_facts:r.scene_facts,guard:r.resource_guard},null,2);state.textContent=r.source==='vlm'?`VLM thật đã trả lời · ${Number(r.latency_ms||0).toFixed(0)} ms · TTS qua ALSA`:'Đã xử lý · '+(r.resource_guard?'đang chờ khe nhiệt an toàn':'TTS qua ALSA')}
