...
function showTurn(j){const r=j.response||j;answer.textContent=r.answer_vi||r.error||'—';facts.textContent=JSON.stringify({source:r.source,thermal_wait_ms:r.thermal_wait_ms,first_token_ms:r.first_token_ms,vlm_latency_ms:r.latency_ms,early_tts_text:r.early_tts_text,frame:r.vlm_input,yolo_depth_scene_facts:r.scene_facts,guard:r.resource_guard},null,2);state.textContent=r.source==='vlm'?`VLM thật · token đầu ${Number(r.first_token_ms||0).toFixed(0)} ms · hoàn tất ${Number(r.latency_ms||0).toFixed(0)} ms · VieNeu streaming`:'Đã xử lý · '+(r.resource_guard?'đang chờ khe nhiệt an toàn':'TTS qua ALSA')}
...
talk.addEventListener('pointerdown',e=>{e.preventDefault();start()});talk.addEventListener('pointerup',e=>{e.preventDefault();stop()});talk.addEventListener('pointerleave',()=>{if(recording)stop()});document.getElementById('ask').onclick=ask;document.getElementById('speak').onclick=speak;setInterval(health,2000);health();
