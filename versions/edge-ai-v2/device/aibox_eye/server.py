...
const sttLab=document.createElement('div');sttLab.className='stt-lab';sttLab.innerHTML='<strong>🎙 Speech-to-Text Lab</strong><div id="sttHealth" class="stt-health">Đang kiểm tra Whisper và VAD…</div><button id="sttTest" class="stt-test">Nhấn giữ để test riêng STT</button><div class="muted">Không gọi Qwen và không phát TTS — phù hợp kiểm tra microphone nhanh.</div>';talk.insertAdjacentElement('afterend',sttLab);const sttTest=document.getElementById('sttTest'),sttHealth=document.getElementById('sttHealth');let sttTesting=false;
...
async function health(){try{const h=await json('/health');lastHealth=h;const t=h.resources?.npu_temperature_c,s=h.components?.stt,v=s?.vad,model=(s?.model_name||s?.model?.split('/').pop()||'Whisper').replace('ggml-','').replace('.bin',''),accelerator=(s?.accelerator||'cpu_neon').replace('_',' ').toUpperCase();state.textContent=`${h.mode} · camera=${h.scene?.camera_ok?'OK':'OFF'} · detector=${Number(h.resources?.detector_fps||0).toFixed(1)} FPS · NPU=${Number(t||0).toFixed(1)}°C · VLM=${h.components?.vlm?.admitted?'sẵn sàng':'đang làm mát (câu hỏi sẽ xếp hàng)'}`;sttHealth.textContent=`${s?.ok?'● Sẵn sàng':'● Lỗi'} · Whisper ${model} · ${accelerator} · final sau khi thả nút · ctx ${Number(s?.audio_context_seconds||0).toFixed(2)} s · VAD ${v?.enabled?'bật':'tắt'} (${Number(v?.threshold||0).toFixed(2)})`;telemetry.textContent=JSON.stringify({scene:h.scene,resources:h.resources,components:h.components},null,2)}catch(e){state.textContent='Lỗi health: '+e.message}}
...
async function startSttTest(){if(recording||sttTesting||busy)return;try{await json('/push-to-talk/start',{method:'POST'});sttTesting=true;sttTest.classList.add('active');sttTest.textContent='Đang thu mic… thả để Whisper nhận dạng';transcript.textContent='Đang nghe…';timing.textContent='STT Lab đang ghi âm 16 kHz mono…'}catch(e){state.textContent='Không mở được microphone: '+e.message}}
async function stopSttTest(){if(!sttTesting)return;sttTesting=false;sttTest.classList.remove('active');sttTest.textContent='Whisper đang xử lý…';busy=true;try{const j=await json('/stt/test/stop',{method:'POST'}),s=j.stt;transcript.textContent=j.transcript_vi||'—';answer.textContent='STT-only: không gọi Qwen/TTS';timing.textContent=[`Transcript: ${j.transcript_vi}`,`Audio microphone: ${s.audio_seconds.toFixed(2)} s · ${s.sample_rate_hz/1000} kHz mono`,`Whisper STT: ${(s.latency_ms/1000).toFixed(2)} s`,`RTF: ${s.real_time_factor.toFixed(2)} (${s.real_time_factor<1?'nhanh hơn realtime':'chậm hơn realtime'})`,`VAD: ${s.vad_enabled?'bật':'tắt'} · backend ${s.backend}`,`Detector vẫn chạy: ${Number(lastHealth?.resources?.detector_fps||0).toFixed(1)} FPS`].join('\n');state.textContent='STT test hoàn tất — transcript và tốc độ đã cập nhật.'}catch(e){state.textContent='STT test lỗi: '+e.message;timing.textContent='Không nhận được tiếng nói. Hãy giữ nút lâu hơn và nói gần microphone.'}finally{busy=false;sttTest.textContent='Nhấn giữ để test riêng STT'}}
...
talk.addEventListener('pointerdown',e=>{e.preventDefault();start()});talk.addEventListener('pointerup',e=>{e.preventDefault();stop()});talk.addEventListener('pointerleave',()=>{if(recording)stop()});sttTest.addEventListener('pointerdown',e=>{e.preventDefault();startSttTest()});sttTest.addEventListener('pointerup',e=>{e.preventDefault();stopSttTest()});sttTest.addEventListener('pointerleave',()=>{if(sttTesting)stopSttTest()});document.getElementById('ask').onclick=ask;document.getElementById('speak').onclick=speak;document.getElementById('testPc').onclick=()=>pcSpeak('Loa máy tính đã hoạt động. Tôi sẵn sàng hỗ trợ bạn.');window.speechSynthesis?.getVoices();setInterval(health,2000);health();
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if parsed.path == "/push-to-talk/stop":
                    ...
                    return
                if parsed.path == "/stt/test/stop": self.send_json(app.stop_stt_test()); return
